...
echo "<pre>";
echo "--- Fixing Product Names (Using 180 Unique Items) ---\n";

// Use the EXACT same list as seed_more_products.php to match user request
// 30 Unique items per category
$templates = [
    'Electronics' => [
...
$stmtUpdate = $pdo->prepare("UPDATE products SET name = ?, slug = ? WHERE id = ?");

// Fetch products joined with categories
$sql = "SELECT p.id, p.name, pc.category_id 
        FROM products p 
        JOIN product_category pc ON p.id = pc.product_id
        WHERE p.id > 1 AND p.id <= 360  -- Limit to first batch of seeded products
        ORDER BY p.id ASC";

// Wait! If user re-ran seed script, IDs might be > 360. 
// Let's just update ALL products > 5 (skipping samples)
$sql = "SELECT p.id, p.name, pc.category_id 
        FROM products p 
        JOIN product_category pc ON p.id = pc.product_id
        WHERE p.id > 5 
...
$updated_count = 0;
// We need to keep track of indices per category to assign 0..29 cyclically
$cat_indices = [];


foreach ($products as $p) {
    if (!isset($cat_map[$p['category_id']]))
        continue; // Skip misc categories

    $cat_name = $cat_map[$p['category_id']];
    if (!isset($cat_indices[$cat_name]))
        $cat_indices[$cat_name] = 0;

    // Explicitly use the array length in case it's not exactly 30
    $count = count($templates[$cat_name]);
    $idx = $cat_indices[$cat_name] % $count;

    $new_name = $templates[$cat_name][$idx]['name'];

    // Increment for next product in this category
    $cat_indices[$cat_name]++;

    // Generate slug
    $slug_base = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $new_name)));
    $new_slug = $slug_base . '-' . uniqid();

    $stmtUpdate->execute([$new_name, $new_slug, $p['id']]);
    echo "Product [#{$p['id']}] ($cat_name) -> '$new_name'\n";
    $updated_count++;
}

echo "--- Complete. Updated $updated_count products with unique names. ---\n";
echo "</pre>";
?>
